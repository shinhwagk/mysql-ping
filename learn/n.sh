...
verbose_log() {
    if [[ "${SHOW_VERBOSE_LOG}" == "true" ]]; then
        printf >&2 "  ${SGR_CYAN}%10s${SGR_RESET} : ${SGR_FAINT}%s${SGR_RESET}\n" "$1" "$2"
    fi
}
...
function show_diagnostics() {
    ...
    if is_ok "${N_NODE_MIRROR}/"; then
        ...
        if command -v curl &>/dev/null; then
            (
                set -x
                do_get --head "${N_NODE_MIRROR}/"
            )
        else
            (
                set -x
                do_get --spider "${N_NODE_MIRROR}/"
            )
            printf "\n"
        fi
    fi
...
while [[ $# -ne 0 ]]; do
    ...
    --latest)
        display_remote_versions latest
        exit
        ;;
    --stable)
        display_remote_versions lts
        exit
        ;; # [sic] old terminology
    --lts)
        display_remote_versions lts
...
if test $# -eq 0; then
    ...
    while test $# -ne 0; do
        ...
        i | install)
            ...
            exit
            ;;
        N_TEST_DISPLAY_LATEST_RESOLVED_VERSION)
            shift
            get_latest_resolved_version "$1" >/dev/null || exit 2
            echo "${g_target_node}"
            exit
            ;;
        *)
